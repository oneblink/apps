...
import { getSchedulingFormSubmissionResult } from '../src/scheduling-service'
import { getSchedulingSubmissionResult } from '../src/services/schedulingHandlers'
import { handlePaymentSubmissionEvent } from '../src/payment-service'

jest.mock('../src/services/schedulingHandlers')
jest.mock('../src/payment-service', () => ({
  ...jest.requireActual('../src/payment-service'),
  handlePaymentSubmissionEvent: jest.fn(),
}))
...
describe('sheduling service', () => {
  describe('getSchedulingFormSubmissionResult', () => {
    it('should throw an error if scheduling result config not found', () => {
      jest.mocked(getSchedulingSubmissionResult).mockResolvedValue(null)
      expect(
    ...
    it('should add payment configuration if there a payment event and paymentReceiptUrl', async () => {
      ...
      jest.mocked(handlePaymentSubmissionEvent).mockResolvedValue({
        amount: 10,
        paymentFormUrl: 'FORM_URL',
        paymentReceiptUrl: 'RECEIPT_URL',
        hostedFormUrl: 'HOSTED_FORM_URL',
