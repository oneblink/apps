...
/**
 * Get a google maps geocode result from a latitude and longitude
 *
 * #### Example
 *
 * ```js
 ...
 * const lng = -87.6562292
 * const formId = 1
 * const result = await formService.getGoogleMapsReverseGeocoding({
 *   lat,
 *   lng,
 *   formId,
 * })
...
export async function getGoogleMapsReverseGeocoding({
  ...
  formId: number
  abortSignal?: AbortSignal
}): Promise<{ reverseGeocodeResult: google.maps.GeocoderResult }> {
  try {
    const urlParams = new URLSearchParams({
      lat: lat.toString(),
      lng: lng.toString(),
    ...
    return await getRequest(
      `${
        tenants.current.apiOrigin
      }/forms/${formId}/google-maps/reverse-geocode?${urlParams.toString()}`,
      abortSignal,
    )
  } catch (err) {
    if (!abortSignal?.aborted) {
